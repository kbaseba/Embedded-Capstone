  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  if (HAL_GetTick() - delay >= 100) {
		  ...
		  // Create buffer for distance reading
		  char dist_str[5];

		  // COnvert distance reading to string (in buffer)
		  sprintf(dist_str, "%.2f", dist1);

		  // Create buffer for json string
		  char json[64];

		  // Implement json string w/needed values
		  strcpy(json, "{ \"range\" : ");
		  ...
		  // Uart transmission to rpi
		  HAL_UART_Transmit(&huart2, json, sizeof(json), 10);
//		  HAL_Delay(5000);
	  }
